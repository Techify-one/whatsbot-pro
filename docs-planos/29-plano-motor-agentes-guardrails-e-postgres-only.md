...
| D4 | **Não está em produção/distribuído** (memória `refactor-rollout-context`) ⇒ **refactor agressivo, sem stopgap de compatibilidade**. Alvo dev/prod é Postgres (memória `postgres-dev-target`, host `10.8.200.13`). | Sem camada de compat SQLite; o EXE Windows zero-config deixa de valer (ver P1). |
...
- **Problemas / pendências:** nenhuma.
- **Verificação:** `pytest tests/` e `tests/test_endpoints.py` (990 checagens) verdes contra `whatsbot_test` (PG 12.22 em 10.8.200.13).
...
| Fail-fast Postgres (C0) | dev sem PG local não sobe mais | mensagem acionável + doc; usar o PG dev compartilhado (`10.8.200.13`) |
