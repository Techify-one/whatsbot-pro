...
| D3 | ✅ (2026-07-20) **HÍBRIDO** escolhido pelo usuário: F1–F3 + F5A (bloquear) como base + F5B (converter) como incremento com degradação graciosa (converte se ffmpeg existir; senão bloqueia). Dockerfile ganha ffmpeg. | Implementadas as duas: `validate_video` bloqueia/roteia, `transcode_to_cloud_mp4` converte quando ffmpeg presente. |
...
|--------|---------------|-------|
| Frontend — menu anexo | [Composer.js:251-262](../web/static/js/components/contacts/Composer.js#L251-L262) | Só dois botões: 🖼 Imagem, 📄 Documento |
...
- **O que foi feito:** Nova rota `POST /api/contacts/{phone}/send-video` em [contacts.py](../server/routes/contacts.py) espelhando `send-image` (mesmas guardas: `permission_denied`, `_inbox_send_denied`, `_is_sandbox_contact`, `_channel_for`, `_session_window_block`); grava em `statics/outbox/`; roteia `kind="video"` via `messaging.send_media`. Novo módulo [channels/video_validate.py](../channels/video_validate.py) (`validate_video(path, caps)` → `VideoVerdict{reason, message, codec_checked}`).
- **Como foi feito / decisões:** Validação **capability-driven** (nunca `if provider ==`): `is_windowed(caps)` = `session_window_hours>0` é o discriminador do Cloud. Always-open (GOWA/Telegram) → `OK` imediato (mandam mp4 como arquivo). Windowed (Cloud) → extensão (.mp4/.3gp), tamanho ≤16 MB e — **se ffprobe existir** — codec H.264/AAC + ≤1 faixa de áudio; sem ffprobe degrada p/ extensão+tamanho. Motivos estruturados → status: `too_big`→413, `bad_format`/`bad_codec`→415. Erro 131053 da Meta vira mensagem amigável (422).
- **Problemas / pendências:** Nenhuma. `os` importado no topo de contacts.py.
- **Verificação:** `py_compile` OK; asserts unitários do `validate_video` (always-open passa, windowed+mkv→bad_format, windowed+>16MB→too_big) passaram standalone.
...
**Estado:** ✅ Concluída (2026-07-20)
- **O que foi feito:** `api.js`: `sendVideo` (contato) + `sandboxSendVideo` (sandbox). `ContactDetail.js`: `sendVideo` no `_api`. `Composer.js`: `videoInputRef` (`accept="video/*"`), item "🎬 Vídeo" no menu de anexo e caso `type==='video'` no overlay de confirmação (`<video controls>`). `useMediaUpload.js`: `pickVideo`/`handleVideoSelected` + branch `video` em `confirmPendingMedia` (bolha otimista `media_type='video'` → `api.sendVideo`). `Sandbox.js` + `sandbox.py`: caminho `send-video` local (sem transcrição, agente reage à legenda).
- **Como foi feito / decisões:** `accept="video/*"` (não só mp4) porque no Híbrido o servidor converte — deixar o operador anexar qualquer vídeo e o backend decidir por capability. Validação client mínima: teto de 200 MB (casa com o teto de entrada do transcode) via `notify(kind:'error')`; o resto é decidido no servidor. Vídeo **não** tem caminho de nota privada → bloqueado com aviso em modo privado. Render da bolha já existia ([MediaContent.js:65](../web/static/js/components/contacts/MediaContent.js#L65)).
- **Problemas / pendências:** Nenhuma. Erros do servidor (F5A) são exibidos via `notify(res.error)` na falha do envio.
- **Verificação:** `node --check` OK em api.js, Composer.js, useMediaUpload.js, ContactDetail.js, Sandbox.js.
...
**Estado:** 🟡 Parcial (2026-07-20) — testes escritos; suíte não rodável neste ambiente
- **O que foi feito:** Teste em [test_endpoints.py](../tests/test_endpoints.py): seção "Send Video" (`POST /send-video` com mp4 → 200 + `send_file` do GOWA chamado) + asserts do `validate_video` (always-open→ok, windowed+mkv→bad_format, windowed+>16MB→too_big). Seção Sandbox: `POST /sandbox/send-video` → 200. Chave `video` já existia no `_MEDIA_METHOD` do Telegram ([:467](../assets/plugin_examples/telegram/channels.py#L467)) → `sendVideo` (nenhuma mudança necessária).
- **Como foi feito / decisões:** O caso de **bloqueio via HTTP** (canal windowed real) exigiria um canal Cloud vivo roteado + janela aberta (setup frágil) — cobri a lógica de política de forma determinística por **asserts unitários** do `validate_video` em vez de montar o canal. O caminho E2E do GOWA prova o roteamento `kind="video"` ponta-a-ponta.
- **Problemas / pendências:** ⚠️ **A suíte pytest NÃO foi executada** — não há Postgres de teste neste ambiente (o `.env` só tem o banco de produção `whatsbot-dev`, e a trava de segurança de [tests/pg.py](../tests/pg.py) corretamente recusa banco sem "test" no nome; não há binário de servidor Postgres nem Docker na máquina). Rodar `venv/bin/python -m pytest tests/ -q` com `WHATSBOT_TEST_DB_URL` apontando p/ um Postgres de teste antes do merge.
- **Verificação:** `py_compile` do test_endpoints.py OK; lógica do `validate_video` validada standalone.
...
**Estado:** ✅ Concluída (2026-07-20)
- **O que foi feito:** Client (`useMediaUpload.js`): teto de 200 MB antes de enfileirar + exibição de `res.error` do servidor via `notify` na falha. Server (rota F1): quando `validate_video` reprova e o transcode não roda, remove o arquivo órfão e retorna `_err` com mensagem legível por motivo (`too_big`→"acima de 16 MB", `bad_format`→"use MP4 (H.264/AAC)", `bad_codec`→"use H.264/AAC") e status 413/415. Erro 131053 da Meta → "O WhatsApp recusou o vídeo (codec/formato). Reexporte em MP4 H.264/AAC." (422).
- **Como foi feito / decisões:** Sem ffprobe em prod (até rebuild da imagem), `bad_codec` não é detectável localmente → o gancho 131053 cobre o caso residual com mensagem amigável (não a string crua).
- **Problemas / pendências:** Nenhuma.
- **Verificação:** `validate_video` retorna mensagens PT-BR corretas por motivo (asserts standalone).
...
**Estado:** ✅ Concluída (2026-07-20)
- **O que foi feito:** [Dockerfile](../Dockerfile): `ffmpeg` (traz `ffprobe`) adicionado ao `apt-get install`. Novo módulo [channels/video_transcode.py](../channels/video_transcode.py): `transcode_to_cloud_mp4(path)` guardado por `shutil.which("ffmpeg")`, ffprobe lê duração/dimensões, calcula bitrate-alvo pela duração p/ caber em ~92% de 16 MB, `libx264 -preset veryfast` + `aac` + `-movflags +faststart -pix_fmt yuv420p`, downscale (long edge ≤1280) só quando necessário, `-ac 2`. Integrado à rota F1: reprovou → tenta transcode; sucesso → move o mp4 pro outbox e envia; falha/sem ffmpeg → cai na Opção A (bloqueio).
- **Como foi feito / decisões:** Degradação graciosa idêntica ao `_gif_to_mp4`. Tetos de entrada defensivos (`MAX_INPUT_BYTES=200MB`, `MAX_INPUT_DURATION_S=300`, timeout 180s) p/ não travar o worker; roda em `asyncio.to_thread`. Só valida (portanto só transcodifica) em canais windowed — GOWA/Telegram mandam o original.
- **Problemas / pendências:** ffmpeg **não está** neste dev box (a afirmação do §2.5 do plano era desatualizada) → o transcode aqui retorna `None` e cai no bloqueio, exatamente o comportamento projetado. Em produção, só funciona após rebuild da imagem Docker (feito no Dockerfile).
- **Verificação:** `py_compile` OK; `video_transcode.available()` retorna `False` sem ffmpeg (degradação confirmada).
...
**Estado:** 🟡 Parcial (2026-07-20) — verificação estática feita; regressão de suíte pendente de Postgres de teste
- **O que foi feito:** Confirmado que nenhum consumidor de `useMediaUpload`/`Composer` além dos 3 arquivos esperados existe; sandbox reusa `ContactDetail` com `sandboxApi` (agora com `sendVideo`). Nenhum `if provider ==` novo — validação e roteamento são capability-driven. Sem segredos em URL/log. Render de vídeo (modo escuro) usa `<video>` sem tinta crua.
- **Como foi feito / decisões:** Verificação por `py_compile` (Python) e `node --check` (JS) em todos os arquivos tocados; lógica de política validada por asserts standalone.
- **Problemas / pendências:** ⚠️ Falta rodar `pytest tests/` num Postgres de teste (indisponível neste ambiente — ver F3). Falta o teste manual E2E contra um canal Cloud real (opcional). Recomendado antes do deploy: (1) rodar a suíte; (2) rebuild da imagem Docker p/ ffmpeg entrar em produção; (3) validar envio de mp4 ≤16 MB a um contato Cloud de teste.
- **Verificação:** estática verde; suíte não executada aqui.
