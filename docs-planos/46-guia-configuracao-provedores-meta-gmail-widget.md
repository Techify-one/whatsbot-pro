...
2. Anote **Instagram App ID** e **Instagram App Secret** (podem diferir do App ID/Secret geral).
3. **Webhook:** em Instagram → Webhooks → **Callback URL** = URL do canal IG no WhatsBot (A0.1) + **Verify Token** = `verify_token` do canal → salvar.
4. **Assinar os campos** (object `instagram`): `messages`, `message_reactions`, `message_echoes`, `messaging_postbacks`, `messaging_seen`, `messaging_referral`, `messaging_optins`, `messaging_handover`, `standby`. (Comentários: `comments`, `live_comments` — opcional, fase 2.)
5. **Token do usuário IG (Business Login):** o modo mais simples p/ começar é gerar/colar um **IG User access token** (long-lived, 60 dias). O `IG_ID` (id da conta profissional) é descobrível via `GET /me?fields=user_id,username`. → campos **`access_token`** e **`ig_id`** no WhatsBot.
6. **Assinar a conta ao app:** `POST /v25.0/{IG_ID}/subscribed_apps` (o WhatsBot faz via botão "Assinar").

**No WhatsBot:** crie um canal **Instagram**, preencha `ig_id`, `access_token`, `app_secret`, `verify_token`. Teste com uma DM **de uma conta testadora**.
...
**⚠️ Token de 60 dias:** o IG User token expira em 60 dias e o WhatsBot **renova automaticamente** (loop de refresh — sub-plano 03). Mas se o canal ficar muito tempo sem uso e o token não for renovado a tempo, ele **morre** e você precisa reautorizar. O WhatsBot mostra "reautorizar" no card quando isso acontece.
