...
> **Origem:** investigação nesta sessão (workflow multi-agente de trade-offs) sobre "a IA re-consulta uma tool cujo resultado ela já tinha". Motivação concreta: no chat MTCSE, a IA chamou `pesquisar_perguntas_frequentes` (cujo resultado já continha a resposta do MEC) e, no turno seguinte, chamou **de novo** para responder "é reconhecido pelo MEC?". Hoje o resultado de uma tool é descartado ENTRE turnos de propósito ([tool_memory.py:53](../agent/tool_memory.py#L53) dá `break` na linha `→`). **Método:** leitura do código real + `grep` exaustivo + workflow de 7 investigadores + crítica adversarial nesta sessão. Todo `arquivo:linha` abaixo foi **verificado**.
