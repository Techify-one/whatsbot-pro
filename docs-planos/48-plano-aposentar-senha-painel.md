...
- **Problemas / pendências:** `test_endpoints.py` (a suíte canônica do plano, `python tests/test_endpoints.py`) está **verde** — as 8 falhas dela são **pré-existentes/alheias** (busca acento do plugin `protocolos` instalado). **Débito de migração da suíte pytest secundária (FOLLOW-UP):** o plano escopou a migração só de `test_endpoints.py`, mas o gate `has_users` quebra qualquer arquivo pytest que **crie usuários E faça chamadas `/api/*` tokenless** (o padrão pré-48 de "modo aberto"). Migrados: `test_lifecycle_characterization` (autenticado como Operador, goldens regenerados). **Ainda a migrar** (mesma técnica — autenticar o client; sem goldens nesses): `tests/endpoints/test_conversation_events_c0.py` (2 falhas em isolamento), `tests/test_utm_atendente.py` (8 falhas em isolamento). Já compatíveis: `test_conversation_read_isolation.py` (usa `_login`), `test_rbac_characterization.py` (desenhado p/ auth — tem 1 falha **pré-existente/flaky**, reproduz idêntica no fork-point 661a590, order-dependent, não é do plano 48). Não foi adotado cache/fixture de conftest (o `teardown_module` por-arquivo funciona, mas a fixture module-autouse do conftest se mostrou não-confiável na limpeza cross-módulo — descartada).
