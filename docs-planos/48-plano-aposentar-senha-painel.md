...
> **Status:** ADIADO (2026-07-15) — a F0 foi iniciada e revelou um custo de migração da suíte de testes muito maior que o estimado (ver "Status de execução — Fase 0"); o usuário optou por entregar o [plano 47](47-plano-trocar-propria-senha.md) primeiro e retomar o 48 como esforço dedicado. O código do gate F0 foi **revertido** (suíte verde). · **Escopo:** médio/grande (mexe em autenticação — alto risco)
...
**Estado:** ⛔ Bloqueada / revertida (2026-07-15) — o **código do app** ficou pronto e correto; o **custo de migração da suíte de testes** inviabilizou seguir sem uma decisão de abordagem, e o usuário optou por adiar o plano 48.
- **O que foi feito (e revertido):** gate do middleware ([app.py:519-541](../server/app.py#L519)) e do WS ([websocket.py:22-25](../server/routes/websocket.py#L22)) ancorados em `has_users`; cache `user_repo.has_any()` invalidado em create/delete. Tudo **revertido** para manter a suíte verde (1240 passed). O código está descrito abaixo para quem retomar.
- **Achado central (custo real):** a suíte `tests/test_endpoints.py` (5226 linhas) foi arquitetada em torno do **modo aberto** (`current_user=None`, default-allow) para os testes funcionais. Com o gate por `has_users`, ela quebra **em cascata**: o 1º usuário nasce na **linha 1206** (crash imediato em `PUT members` sem token → 401 → `r.json()["data"]` estoura); o **admin criado na linha 1305 nunca é deletado** (guard de "último admin"), então `has_users` fica `True` por **~75% da suíte**; há **~10 sites** que criam usuários. Não existe correção pequena — exige migrar a suíte para autenticar.
- **Abordagens avaliadas (para retomar):**
  - **(A) Migrar a suíte** mantendo o modelo `has_users` (fiel ao D2): header admin default no `client` + um `TestClient` anônimo para as ~10 checagens sem-token + corrigir asserções semânticas que mudam de `None`→admin (autor de auditoria [linha 5176], escopo de `saved_filters` com `user_id` NULL, `sent_by`). Churn alto; usar a suíte como oráculo, iterativo. ⚠️ Conflito: bootstrap-no-topo colide com os testes de bootstrap (1305) — o header default precisa entrar **depois** do bootstrap, e há user já em 1206.
  - **(B) Flag de enforcement** — trocar o gate por `rbac_enforce` com **default LIGADO** (dropar `has_users`); a suíte seta a flag `False` uma vez no topo (1 linha) e mantém o modo aberto; uma seção dedicada liga para testar 401. Churn mínimo; **desvia do D2** (flag em vez de self-healing) e força login em todo install já no F0. Cuidado com o fluxo pré-bootstrap/wizard (que o `has_users` cobria de graça).
- **Problemas / pendências:** decidir A vs B antes de retomar. O restante do plano (F1/F2/F3) depende de F0 estar fechado.
- **Verificação:** com F0 revertido, `venv/bin/python tests/test_endpoints.py` → **1240 passed, 0 failed**.
...
**Estado:** ⬜ Não iniciada
- **O que foi feito:** _(preencher)_
- **Como foi feito / decisões:** _(mensagem do login sem email; destino de has_password — P1)_
- **Problemas / pendências:** _()_
- **Verificação:** _(grep limpo; login email+senha ok; suíte verde; sessões legadas caem — esperado)_
...
- **P5 — Custo de `user_repo.count()` por request.** ⏸️ O gate roda em todo `/api/*`. Opções: (a) cache curto em memória (TTL ~30s, invalidado no create/delete de usuário); (b) query direta (é `SELECT count(*)`, barata). **Recomendação:** (a) leve, para não bater no DB a cada request; medir antes de otimizar.
