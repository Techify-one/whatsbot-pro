...
- **O que foi feito:**
  - `user_repo.has_any()` + `invalidate_has_any_cache()` ([user_repo.py](../db/repositories/user_repo.py)) — cache **invalidation-based** (create→True implícito via reset, delete→reset), sem janela de staleness; invalidado em `create`/`delete`.
  - Gate do middleware ([app.py](../server/app.py), bloco `/api/*`): `has_users = user_repo.has_any()`; `enforce = rbac_enforced OR has_users`; `auth_required` mantido como 3ª condição OR (D5); `denied = kind != "user"` quando enforce. Import `user_repo` adicionado.
...
- **Problemas / pendências:** nenhuma para F0. As **8 falhas** remanescentes da suíte são **pré-existentes** e alheias (busca acento/caixa do plugin `protocolos` instalado — gotcha do plugin em `storages/`, não do core/auth); confirmado que nenhum arquivo de busca/protocolos foi tocado.
