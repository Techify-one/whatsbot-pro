...
> **Método:** investigação nesta sessão — leitura do código real (`arquivo:linha` abaixo), sub-agente `Explore` para o fluxo de QR, e cruzamento com o print do usuário (canal `teste_gowa`: `Desconectado / Autenticado / 554498510557`). **Sem GOWA vivo** nesta máquina — o gatilho exato do bug #2 (ALREADY_LOGGED_IN vs DEVICE_NOT_FOUND) fica marcado "a confirmar em log" (P1), mas o fix cobre os dois.
>
...
⚠️ **Resultado observado:** `teste_gowa` mostra `Autenticado` + `📱 554498510557` (⇒ `is_connected()`=True ⇒ `logged_in`) mas `Desconectado` (⇒ `manager.is_running`=False). Textbook do bug #2.
