...
|---|---------|------------------------|
| **D1** ✅ (2026-07-07) | **Ler o `ExemploDB` direto** (2ª conexão SQLAlchemy read-only nas settings do plugin). NÃO replicar CRUD de ofertas/produtos/FAQ. | Nenhuma tabela de catálogo no WhatsBot. A busca vetorial roda **no servidor do Nexus** ⇒ o Postgres do WhatsBot **não precisa de pgvector**. |
...
**Banco Nexus (`ExemploDB`, schema `public`)** — o read-side (trust `\d+`, o Prisma está desatualizado):
...
**Objetivo:** conexão isolada e read-only ao `ExemploDB`, com helpers de query.
**Itens:**
...
- Banco `ExemploDB@203.0.113.30`: `produtos_{ofertas,produtos,faq}`, `gerenciamento_ia_embeddings_{oferta,curso,faq}`, `gerenciamento_ia_prompts`
