...
**Estado:** ✅ Concluída (2026-07-16)
- **O que foi feito:** unit systemd `whatsbot-ai-server.service` (Restart=always, User=whatsbot-ai, --env-file), .env chmod 600 com secret `openssl rand -hex 32`; MESMO secret colado na config do gateway (`plugin.melhorias.ai_server_secret`) + URL; monitor: /health público, /auth-check p/ secret (botão Testar conexão na seção de IA do plugin).
- **Como foi feito / decisoes:** systemd (o :8014 já usava); OAuth ~/.claude compartilhado (já logado — serve os dois).
- **Problemas / pendencias:** rate-limit no gateway fica p/ v2. ⚠️ GOTCHA descoberto no 1º uso real: o CLI do Agent SDK RECUSA `bypassPermissions` rodando como ROOT e sai com código 1 ("Claude Code process exited with code 1"). Solução definitiva (2026-07-16): serviço movido para `/opt/whatsbot-ai-server` rodando como o usuário dedicado **`whatsbot-ai`** (criado; OAuth seedado em /home/whatsbot-ai/.claude — o relogin do :8015 renova a cópia dele), SEM IS_SANDBOX (que foi o paliativo intermediário; o :8014 segue root+IS_SANDBOX). Runner loga erro no journal (o frame SSE de erro pode se perder se chegar antes do consumidor). Send do gateway faz auto-resume+retry em 404 do executor (runner morto). Conversa real validada como não-root (message_start/chunk/end).
