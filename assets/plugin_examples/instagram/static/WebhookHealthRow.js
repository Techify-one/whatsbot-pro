// Instagram — WebhookHealthRow (espelha o do facebook_messenger). Linha de saúde
// do webhook que aparece SÓ nos cards de canal ``instagram``. Registrada no slot
// genérico ``channel.card.rows`` pelo extends.js deste plugin.
//
// Self-fetch (não toca o status em massa): no mount, pergunta à Meta qual callback
// está configurado no webhook ``object=instagram`` do app e compara com a URL que
// ESTA instância espera (``${origin}/api/webhook/instagram/${channel.id}``). O
// botão "Configurar webhook" repointa o callback (com confirmação) e re-checa —
// mesma mecânica do /autoconfigure, um clique.
//
// `http` chega por prop do extends.js (buildPluginHttp('/api/plugins/instagram')).
...
export function WebhookHealthRow({ channel, http }) {
  // Hard gate: só cards de canal Instagram. O slot renderiza para TODO provider;
  // o filtro por provider aqui DENTRO do plugin é legítimo. Hooks abaixo são
  // incondicionais (o provider nunca muda para um card montado), então o
  // early-return é seguro.
  if (channel.provider !== 'instagram') return null;

  const expectedUrl = `${window.location.origin}/api/webhook/instagram/${channel.id}`;
  ...
  async function handleSet() {
    if (!confirm('Apontar o webhook desta conta do Instagram para ESTA instância? Isso substitui o callback atual do app na Meta — outro sistema que use o mesmo app para webhooks de Instagram deixa de receber.')) return;
  ...
  return html`
    <div class=${`text-[12px] ${meta.tint} border rounded-md px-2 py-1.5 mt-2 break-words`}>
      ...
      ${blockedNeedsCreds ? html`
        <div class="mt-1 opacity-80">Preencha o App Secret e o Verify Token em Editar para apontar o webhook automaticamente (o App ID é detectado sozinho).</div>
      ` : null}
      ${!state.subscribed && state.match === 'ok' ? html`
        <div class="mt-1 opacity-80">Callback ok, mas a Página ainda não está assinada — confira a conta do Instagram conectada à Página e o acesso a mensagens permitido.</div>
      ` : null}
