-- Visualizacoes personalizadas do Kanban de Atendimentos (abas de "Agrupar por").
--
...
-- veem). Criar/editar visualizacao de EQUIPE exige a permissao plugin.atendimentos.
...
-- usuario. group_attr_key so e usado quando group_by='attr' (attribute_key de conversa,
-- type=list). group_date_mode so e usado quando group_by='data' (dia|faixas|mes).
--
-- SQL portavel: o migrator valida o prefixo plugin_atendimentos_, traduz
...
CREATE TABLE IF NOT EXISTS plugin_atendimentos_kanban_views (
...
CREATE INDEX IF NOT EXISTS plugin_atendimentos_kv_scope
    ON plugin_atendimentos_kanban_views(scope);
CREATE INDEX IF NOT EXISTS plugin_atendimentos_kv_owner
    ON plugin_atendimentos_kanban_views(owner_user_id);
CREATE INDEX IF NOT EXISTS plugin_atendimentos_kv_pos
    ON plugin_atendimentos_kanban_views(position);
