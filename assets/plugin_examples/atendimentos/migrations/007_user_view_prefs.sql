...
-- (os compartilhados, na coluna filters de plugin_atendimentos_kanban_views, definidos por
...
CREATE TABLE IF NOT EXISTS plugin_atendimentos_user_view_prefs (
    id               INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id          INTEGER NOT NULL,                 -- core users.id do dono da preferencia
    view_id          INTEGER NOT NULL,                 -- plugin_atendimentos_kanban_views.id
...
CREATE UNIQUE INDEX IF NOT EXISTS plugin_atendimentos_uvp_uniq
    ON plugin_atendimentos_user_view_prefs(user_id, view_id);

-- Lookup por usuario ao anexar a preferencia na listagem de visualizacoes.
CREATE INDEX IF NOT EXISTS plugin_atendimentos_uvp_user
    ON plugin_atendimentos_user_view_prefs(user_id);

-- Lookup por visualizacao ao limpar preferencias no delete da visualizacao.
CREATE INDEX IF NOT EXISTS plugin_atendimentos_uvp_view
    ON plugin_atendimentos_user_view_prefs(view_id);
