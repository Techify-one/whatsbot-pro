    ...
    <div draggable=${canDrag}
      ...
      <div class="flex items-center justify-between gap-2 mt-1">
        ${(() => {
          ...
          // (em vez do antigo "Não atribuído"), sem distinguir de um atendente salvo.
...
function DetailModal({ data, fieldDefs = [], protoDefs = [], warning = '',
  ...
  // O rótulo Atendente é a exceção: exibe o atendente EFETIVO (na falta do salvo, o dono
  // da conversa), que não vive em `vals` — senão sumiria da tela sem o assignee gravado.
  const readOnlyInfo = protoDefs.filter((d) => (d.type === 'atendente'
    ? effectiveAssignee(at).id != null
    : (d.type === 'checkbox' || isFilledProto(d, vals[d.key]))));
            ...
            ${readOnlyInfo.length ? html`
              <div class="flex flex-wrap gap-x-5 gap-y-1 text-[12px]">
                ${readOnlyInfo.map((d) => html`<div key=${d.key}>
                  <span class="text-wa-secondary">${d.label}:</span> <span class="text-wa-text">${d.type === 'atendente' ? (effectiveAssignee(at).name || '—') : fmtCell(vals[d.key], d)}</span>
                </div>`)}
