...
* :func:`golden_assert` — serialize ``value`` (json, sorted keys) and compare
  against ``tests/characterization/goldens/<name>.json``; (re)write it when
...
def golden_assert(name: str, value: Any, *,
    ...
    and compares against ``tests/characterization/goldens/<name>.json``. When
    ...
    instead of asserting — so an intentional change is a one-command refresh:
        ...
        UPDATE_GOLDENS=1 ./venv/bin/pytest tests/characterization/...
