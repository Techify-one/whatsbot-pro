...
``tests/test_plano75_parse_inbound.py`` prova o PARSE (``context.id`` →
...
``tests/support.py``) copia o plugin de ``assets/plugin_examples/<id>/`` para um
``storages/plugins`` temporário. Ou seja, o que roda aqui é a fonte COM a
correção da F9 — a cópia eventualmente instalada em ``storages/plugins/`` (que
pode ser a versão antiga do upstream) nunca é usada por estes testes.

    venv/bin/python -m pytest tests/test_plano75_reply_e2e.py -q
"""
