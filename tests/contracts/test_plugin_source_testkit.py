...
def test_resolve_plugin_source_prefers_external_repository(tmp_path):
    external = tmp_path / "whatsbot-pro-plugins" / "plugins"
    examples = tmp_path / "assets" / "plugin_examples"
    installed = tmp_path / "storages" / "plugins"
    expected = _plugin_dir(external / "demo", "src")
    _plugin_dir(examples, "demo")
    _plugin_dir(installed, "demo")
