...
WARN-only (never hard-fail in A0):
    * duplicate SEQUENCE prefixes among version filenames. There is a KNOWN
      duplicate pair that is OUT OF SCOPE for A0 (fixed in a later phase):
      ``..._0021_inbox_members`` and ``..._0021_template_permissions``. That
      pair is allowlisted. Any OTHER duplicate sequence prefix is a real
