...
Todo o resto da API é *pull*. Um CRM que precise saber "chegou mensagem" ou "conversa resolvida" teria de fazer polling — o `/ws` exige sessão de painel e é escopado à audiência atual de cada conversa (contrato completo do roteamento/descarte em [docs/PLUGIN_BUS.md](PLUGIN_BUS.md#audiência-de-conversa-no-websocket-plano-16601--168)), portanto não substitui um webhook de integração.
