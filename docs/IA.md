...
⚠️ **Tool `kind='code'` roda num subprocesso DESCARTÁVEL que congela o servidor enquanto dura** (plano 163) — `agent/tool_isolation.py` → `python -m agent.tool_runner`, chamado de forma **bloqueante** no loop principal ([agent/agno_engine.py](../agent/agno_engine.py)); como `main.py` sobe um processo uvicorn único, o servidor inteiro fica parado até a tool devolver. Três consequências que não têm contorno dentro da própria tool:

- **Não existe timer que sobreviva ao fim da chamada.** `asyncio.get_running_loop().call_later(...)` parece adiar, mas o subprocesso morre com o retorno da função — o "atraso" nunca dispara, ou dispara cedo demais se o código cair no ramo "sem loop ⇒ roda na hora". Um efeito que precisa esperar algo (a resposta ser enviada, a despedida ser salva) **não pode** ser feito por timer dentro da tool.
- **Barramento, serviços e WS não existem lá.** `plugins.events.emit_with_filter_sync`, `services.call(...)` e o broadcast do painel viram no-op silencioso: o registry de plugins não está carregado no subprocesso. Fechar algo (conversa, protocolo) por essa via nunca chega ao Trackify nem ao WS do painel.
- **O custo é medido por import.** Python + `sqlalchemy` + `db.engine` ≈ 0,36s; acrescentar o import de um módulo "pesado" de plugin (que puxa `plugins.context`, outros repos, etc.) pode passar de 1s — medido em produção, uma chamada real chegou a 1,1s. Um módulo FOLHA (só o que a tool precisa, sem puxar o resto do plugin) é o que mantém isso baixo.

Quando o efeito de uma tool `code` precisa sobreviver ao fim da chamada (fechar algo alguns segundos depois, esperar uma condição), o padrão é: a tool **agenda** (grava uma linha numa fila do banco, via um módulo folha do plugin dono do efeito) e uma **task supervisionada dentro do servidor** (`ctx.spawn_task`, como as do `lifecycle.py` de um plugin) executa de verdade, com barramento/serviços/WS disponíveis. Referência completa: `storages/plugins/protocolos/fila_fechamento.py` (a ponte) + `fechamento_agendado.py` (o executor).
