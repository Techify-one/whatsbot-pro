...
## Quanto falta para a janela do cliente fechar (plano 159)

Na linha do seletor **Responder / Mensagem Privada**, um chip compacto diz **"Janela: 5h 12min"** e conta ao vivo — ao lado do seletor, e não uma linha inteira: é informação de relance, não aviso (ajuste pedido depois da 1ª versão, que ocupava uma faixa própria). É **mutuamente exclusivo por construção** com o aviso "Fora da janela", que continua sendo a faixa larga de sempre, em [Composer.js](../web/static/js/components/contacts/Composer.js).

- **São TRÊS janelas no produto e esta é UMA delas: a do ATENDENTE.** `session_window_hours` (24h no Cloud), `human_window_hours` (7 dias no Messenger/Instagram com a tag `HUMAN_AGENT`) e `ai_window_hours` (a IA cala às 24h) são independentes — ver [CANAIS_META.md](CANAIS_META.md). A faixa segue a janela **efetiva do atendente**, que é a que o compositor obedece; a da IA continua governando só os toggles de instrução. Derivar uma da outra reintroduz bug conhecido.
- ⚠️ **O tamanho vem do SERVIDOR, nunca de um "24" escrito no cliente.** O payload da conversa ganhou `last_inbound_ts` + `session_window_hours` **efetivo** ([conversations.py](../server/routes/conversations.py), [contacts.py](../server/routes/contacts.py)), os dois saindo de `OutboundRouter.window_hours(channel_id, by_human=True)` — o **mesmo** helper de onde o `session_open` tira o veredito. Recalcular `max(session, human)` na rota faria painel e envio discordarem sobre a mesma regra na primeira vez que uma das duas mudasse. `0` (GOWA/Telegram: sem janela) é valor VÁLIDO e significa "não exiba nada"; é distinguível de ausente (core anterior) porque o campo trafega **cru**.
- ⚠️ **`shapeConvData` é WHITELIST** ([conversationRows.js](../web/static/js/services/conversationRows.js)): campo novo que não for listado lá **some sem erro nenhum** — foi exatamente assim que o `ai_window_open` se perdeu no caminho.
- ⚠️ **O carimbo acompanha o inbound no WS.** Em [useConversationWsEvents.js](../web/static/js/components/contacts/hooks/useConversationWsEvents.js), o mesmo ponto que marca `session_open: true` grava `last_inbound_ts = message.ts`; sem isso o contador seguiria medindo o inbound **anterior** e iria a zero numa janela que o cliente acabou de reabrir. O tamanho da janela **não** muda ali: é do canal, não da mensagem.
- **O QUANTO é puro, o RELÓGIO não.** A aritmética mora em [sessionWindow.js](../web/static/js/services/sessionWindow.js) (`windowState({lastInboundTs, windowHours, now})` → `{applies, open, remainingMs, label, urgent}`), sem `Date.now()` interno — é o que a torna testável em `node --test`. O `setInterval` de 60s mora no componente e **só existe enquanto há o que contar**. O rótulo usa no máximo duas unidades e **arredonda sempre para baixo**: anunciar mais tempo do que resta é o único erro que custa uma janela. Abaixo de 1h a faixa fica âmbar.

## Atalhos conversa ⇄ protocolo (plano 159)

O atendente atravessa as duas telas o dia inteiro; os dois atalhos são do plugin `protocolos`, e o core só abriu o slot (acima) e os dois campos da janela.

- **Conversa → protocolo**: botão "Protocolo" **na barra**, ao lado do Resolver, via `conversation.header.primary`. A conversa com protocolo **fechado** também tem botão (é o último protocolo dela); conversa sem protocolo nenhum não tem. A rota é nova (`GET /api/plugins/protocolos/conversas/{id}/protocolo`) porque a irmã `/contacts/{id}/protocolo` responde outra pergunta — o protocolo **ABERTO** do par contato+canal, semântica que o plano 123 F6 fixou para o popup de Resolver. O botão carrega uma bolinha de status (plano 161) — verde = aberto, vermelha = fechado — que assina `plugin_protocolos_changed` para não ficar desatualizada se o próprio atendente resolver pelo botão vizinho ou outra aba/atendente mudar o protocolo.
- **Protocolo → conversa**: ícone no card do Kanban, ancorado na 1ª mensagem do atendimento (`?message=`). ⚠️ O alvo (`goto_conversation_id`/`goto_cycle_id`/`goto_message_id`) é **separado** do `latest` que alimenta `atendimento_fields` — este último **filtra e agrupa** o Kanban, e reordenar a query dele para "priorizar o ciclo aberto" mudaria colunas e filtros salvos de todos os usuários em silêncio. ⚠️ **Três gestos disputam o mesmo pixel** no card (arrasto que move de coluna, clique que abre o detalhe, o `draggedRef` que distingue os dois): o `<a>` sai com `draggable="false"` e `stopPropagation` em click e dragstart.
- **Detalhe → conversa** (plano 160): a tabela de atendimentos do popup (`AtendimentosTable`) recebe o alvo pronto por linha (`anchor_message_id`, resolvido em lote por `logic.list_atendimentos`) e trata Ctrl/⌘+clique e clique do meio como o card — nova guia, sem `await` antes do `window.open`. Clique simples continua fechando o popup e navegando na mesma guia.
...
- ⚠️ **Essa folga é só da ESTATÍSTICA — não da âncora de scroll.** Foi cogitado aplicar a mesma folga em `cycle_anchor`/`_cycle_anchors_batch` (o alvo "ir à conversa" do plano 159); quebrou 3 testes que travam o comportamento atual de propósito (a âncora é a 1ª mensagem **a partir** do `started_at`, sem folga, para nunca ancorar numa mensagem do ciclo/protocolo anterior). **Não reabrir sem revisitar esses testes.**
