...
- **Conversa → protocolo**: botão "Protocolo" **na barra**, ao lado do Resolver, via `conversation.header.primary`. A conversa com protocolo **fechado** também tem botão (é o último protocolo dela); conversa sem protocolo nenhum não tem. A rota é nova (`GET /api/plugins/protocolos/conversas/{id}/protocolo`) porque a irmã `/contacts/{id}/protocolo` responde outra pergunta — o protocolo **ABERTO** do par contato+canal, semântica que o plano 123 F6 fixou para o popup de Resolver.
