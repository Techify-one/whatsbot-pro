...
- **Plugin `facebook_messenger`** (fonte publicada em `whatsbot-pro-plugins/plugins/facebook_messenger/src/`; espelho transitório em [assets/plugin_examples/facebook_messenger/](assets/plugin_examples/facebook_messenger/)): não é auto-instalado; o ZIP é gerado no repositório externo. Credenciais `page_id`/`page_access_token`/`app_secret`/`verify_token`; configs `graph_api_version` (v25.0) e `human_agent_tag`; dedup por `page_id`; `contact_type` = `facebook`; screen `config:true` para copiar a URL de callback e assinar o webhook da Página (`POST /{page_id}/subscribed_apps`). Identidade do contato = **PSID page-scoped** (a chave `(channel_id, phone)` do core já cobre).
...
- **Bundling** ([plugins/bootstrap.py](plugins/bootstrap.py) `BUNDLED_AUTO_INSTALL = ("gowa",)`): fresh install copia **só GOWA**. As outras fontes e seus testes pertencem ao repositório `whatsbot-pro-plugins`; `assets/plugin_examples/` permanece temporariamente como espelho da suíte legada. Instalações existentes em `storages/plugins/` ficam intactas.
...
1. **Bootstrap**: com `storages/plugins/` vazio, `plugins.bootstrap.bootstrap_initial_plugins()` semeia **somente `gowa`** a partir de `assets/plugin_examples/gowa/` (`BUNDLED_AUTO_INSTALL`). Os demais diretórios de `assets/plugin_examples/` são bancada versionada e não são instalados automaticamente.
...
- **`plugin_services_version`**: obrigatório em plugin novo que declare `frontend_extends` (use `">=2.0,<3.0"` hoje). Omissão significa legado 1.x; é independente de `frontend_api_version`.
...
`assets/plugin_examples/` ainda existe neste core como **espelho transitório de compatibilidade** para o GOWA bundled e para partes da suíte legada que ainda estão sendo desacopladas. Não é o lugar para desenvolver novos plugins. Enquanto a suíte legada depender desse espelho, mudanças no GOWA devem manter a cópia publicada sincronizada; os demais plugins devem ser editados primeiro no repositório externo.

### Plugin `retornos` — follow-up por configuração de regras (plano 76)

Follow-up automático **configurável pelo operador**, porte do módulo Retornos do Nexus. Fonte em [assets/plugin_examples/retornos/](assets/plugin_examples/retornos/) (versionada aqui, testada junto do core; **não** auto-instalada — `BUNDLED_AUTO_INSTALL` segue só com `gowa`). Substitui a função de follow-up automático do `retorno_automatico` e convive com o `agendamento_retorno` (retorno MANUAL do atendente). ⚠️ Este plugin **não mexe em outro plugin**: o `setup()` já desativou o `retorno_automatico` até a 1.13.0 (setting `disable_retorno_automatico`), e a 1.14.0 removeu as duas coisas — com os dois ativos a conversa recebe **nota duplicada**, e desativar o antigo é decisão do operador em Gerenciar Plugins.

⚠️ **Vocabulário renomeado**: o que o plano 76 chama de **régua** é hoje **configuração**, e o que ele chama de **passo** é hoje **retorno** — em tudo (UI, código, rotas REST, tabelas e colunas). A migration `003_rename_regua_passo.sql` faz `RENAME` do esquema criado por `001`/`002` (nenhum dado se perde; `001`/`002` continuam com os nomes antigos, por serem histórico imutável). No **monitor**, a linha de uma conversa em follow-up chama-se **agendamento** (o registro em `plugin_retornos_controle`), para não colidir com o "retorno" (o passo) nem com o nome do plugin. O plano 76 em `docs-planos/` **não** foi reescrito — é registro histórico.

- **Modelo**: uma **configuração** = **sequência de retornos** (sem gate de entrada — a coluna `filtros_entrada` foi removida na migration `002`; a 1ª configuração ATIVA por `posicao` assume a conversa e **quem filtra são as regras de cada retorno**, avaliadas no disparo com dados frescos); cada retorno = árvore de regras própria + N mensagens (`text`/`private_note`/`image`/`audio`/`video`/`document`/**`ia_responde_agora`**). O **anexo tem de casar com o tipo** ([media_kinds.py](assets/plugin_examples/retornos/media_kinds.py) + espelho [static/mediaKinds.js](assets/plugin_examples/retornos/static/mediaKinds.js), mesmos vetores nos dois testes): `image`/`audio`/`video` só aceitam aquela categoria, `document` aceita tudo; checado ao escolher (`accept` + pré-checagem), no `POST /upload` (415) e **ao salvar a mensagem** (415 — pega quem troca só o tipo depois de anexar). Só bloqueia com identificação POSITIVA de outra categoria (MIME específico, senão extensão); indeterminado passa e quem julga é o provedor (`MediaLimits`). Sem isso o erro só apareceria no disparo: o `tipo` vira o `kind` do `send_media` e o provedor recusa o `.mp4` declarado como imagem. O **A/B é do RETORNO** (coluna `ab_ativo`, migration `004` — antes era um flag por mensagem): ligado, cada disparo envia **UMA** das mensagens do retorno, alternando (rotação atômica pelo cursor `proxima_mensagem_index`); desligado, todas saem na ordem. A **pausa entre as mensagens** também é do RETORNO (coluna `delay_mensagens_seg`, migration `005`, em SEGUNDOS, campo no bloco "Mensagens deste retorno" que salva sozinho no blur): `NULL` = herda a setting global `delay_between_messages_seconds` (estado de todo retorno anterior à 1.6.0), valor próprio ganha, teto de 300 s por mensagem e 300 s por disparo (`MAX_PAUSA_TOTAL_SEGUNDOS` — o ciclo é serial, pausa longa segura os outros agendamentos). Durante a pausa o lock é renovado a cada 30 s (`_pausar`): sem esse heartbeat o `recover_stale_locks` (5 min) devolveria o agendamento à fila no meio do disparo e o cliente receberia a mensagem duplicada. Tabelas `plugin_retornos_{configuracoes,retornos,mensagens,controle,log}`; **um controle por conversa** (`conversation_id` UNIQUE). Placeholder de mensagem: `{retorno}` (o antigo `{passo}` segue como alias).
- **Motor de regras** ([rules.py](assets/plugin_examples/retornos/rules.py) + espelho JS [static/rules.js](assets/plugin_examples/retornos/static/rules.js)): árvore `{regras:[Condicao|Grupo]}` avaliada **esquerda→direita** (`acc = conector=='OU' ? acc or r : acc and r`, sem precedência), **grupos são parênteses e recursam** (profundidade livre). O `conector` de um item diz como ele se junta ao item **anterior** — o 1º da lista não tem conector. Árvore vazia ⇒ **true**; campo fora do catálogo ⇒ **false** (import de árvore do Nexus não quebra). Vocabulário em [catalog.py](assets/plugin_examples/retornos/catalog.py) (ids `chatwoot.*`/`virtual.*`/`modulo.*` preservados por compatibilidade de import/export; `wb.*` são os campos só-WhatsBot). Os dois motores usam os MESMOS vetores de teste — divergir = o preview da UI mentir.
- **Ciclo** ([dispatcher.py](assets/plugin_examples/retornos/dispatcher.py), task `retornos:scheduler` a cada 60s): recovery de lock (`processing` > 5 min) → **lock atômico** (`UPDATE … RETURNING`) → grace window (atrasado além do teto ⇒ cancela) → avalia com dados **frescos**. Regra falsa ⇒ **reagenda o MESMO retorno** (`tentativas_retorno+1`, nunca avança fingindo progresso) até o teto GLOBAL de segurança ⇒ `expired`. Esse teto são as settings do plugin `max_attempts_per_retorno` (60) e `retorno_deadline_minutes` (4320): desde a migration `007` o retorno não tem mais espera/tentativas/prazo próprios (os três campos saíram do formulário) — *quando* disparar é decisão das CONDIÇÕES, avaliadas com dados frescos, e o teto existe só para um agendamento cujas condições nunca batem não reavaliar eternamente. Disparo só **CONTA** se ≥1 mensagem saiu.
- **Envio** ([actions.py](assets/plugin_examples/retornos/actions.py)): canal re-derivado da conversa a cada disparo (nunca o snapshot). **Fora da janela de 24h** (`outbound_router.session_open == False`) a ação vira **nota privada de aviso** (sem template HSM), cujo texto é a setting global `janela_24h_message` (modal "Configurar" do plugin, placeholders `{cliente}`/`{primeiro_nome}`/`{retorno}`/`{disparos}`; **em branco ⇒ nenhuma nota** — o retorno é pulado em silêncio, e o disparo segue contando como feito). `ia_responde_agora` chama `agent_handler.aprocess_message` (a instrução é o turno sintético) e ENVIA a resposta; **não** re-checa o `auto_reply` global, mas checa sempre os gates da conversa (IA off / humano sem agente / tag `transferido_atendente`) e, bloqueado, escreve nota privada. Disparando de fato, escreve **ANTES** do envio uma **nota privada** com a instrução INTEIRA (o painel recolhe nota privada longa — ver `isCollapsibleCard` em [messageView.js](web/static/js/services/messageView.js)). A ordem é funcional, não cosmética: `run_turn` ([agent_run_service.py](app/services/agent_run_service.py)) só entrega o texto ao modelo pelo **histórico**, e com `save_user_message=False` o turno sintético é descartado — então é a nota (`private_note` **entra no contexto do LLM**, não está na lista-negra de roles do `message_repo`) que carrega a instrução até a IA, mesmo desenho do "@ia" privado do core (`_run_private_ai` salva a nota e só então roda a IA). Escrita depois (como era até a 1.5.1), a instrução não chegava ao modelo em chamada nenhuma — a IA respondia só pelo histórico — e o atendente lia o pedido depois da resposta. A nota é também o único rastro no fio de que a mensagem nasceu de um retorno; se a IA não responder, uma 2ª nota curta registra isso. **Supressão de eco**: todo envio deste plugin registra `state.recently_sent` (pelo corpo, ANTES do envio) e `state.processed_messages` (pelo id externo, DEPOIS — cobre mídia sem legenda). Sem isso o envio direto pelo `outbound_router` passa fora do registro que `messaging_service._send_reply` faz, e o `_ingest_echo` grava a MESMA mensagem outra vez com `status='operator'` — a bolha duplicada rotulada "Manual" em canais que ecoam o próprio envio (Messenger, GOWA).
- **Entrada/saída da configuração** ([events.py](assets/plugin_examples/retornos/events.py)): `message.saved` (cliente) cria o controle na 1ª configuração ativa por `posicao`, ou aplica `on_reply` (`reset`|`cancel`) no controle existente; `conversation.status_changed`/`.archived`/`.assigned`/`.ai_toggled` e `contact.ai_toggled` cancelam conforme os toggles da configuração. **Não** assina `message.sent` (laço).
- **Campos de OUTROS plugins** ([contrib.py](assets/plugin_examples/retornos/contrib.py)): o construtor de regras é extensível e o `retornos` **não conhece plugin nenhum por nome** — ele PUBLICA dois filtros no bus e consome o que voltar (nomes fora do `KNOWN_FILTERS` do core, que cataloga só o que o CORE aplica; o bus loga um WARNING informativo ao registrá-los, é esperado). `filter.retornos.campos` recebe `{grupos, campos, opcoes}` e o plugin ACRESCENTA os seus; `filter.retornos.contexto` recebe `{}` e o plugin devolve `{campo_id: valor}` da conversa (`ctx.extras` traz `conversation`/`contact`/`conversation_id`/`contact_id`/`phone`/`channel_id`/`now`). O que volta é **saneado** antes de entrar (grupo/campo que colida com o core é descartado — o core sempre ganha; campo órfão de grupo, fora; tipo desconhecido vira `text`) e registrado em `catalog.registrar_externos`, que é o que faz o MOTOR reconhecer o tipo do campo (sem isso a condição cairia em "campo desconhecido ⇒ falso"). Cache de 30s, `force=True` ao abrir a tela. **Gotcha**: `apply_filter_sync` é inerte na thread do event loop — quem roda lá (as rotas `/metadata` e `/preview`) usa `asnapshot`/`acontexto` e passa `extra=` a `build_ctx`; o dispatcher e os eventos (em thread) usam o caminho síncrono. Plugin desativado ⇒ o filter cai no restart e os campos somem do select sozinhos. Consumidor hoje: **`protocolos`** ([retornos_fields.py](assets/plugin_examples/protocolos/retornos_fields.py)) — grupos "Protocolos" e "Protocolos · Atendimento" com status, atendente EFETIVO, aberto por, tempo desde abertura/fechamento, dia de fechamento (`date`, epoch cru como os campos de data do core), nº de atendimentos, nota da avaliação e **todos os campos configuráveis dos dois escopos** (`protocolos.campo.<key>` / `protocolos.atend.campo.<key>`); o protocolo avaliado é o ABERTO do contato e, na falta dele, o último FECHADO (`protocolos.tem_aberto` distingue).
- **Fuso** = offset FIXO por configuração (`tz_offset_hours`, default −3), usado por todas as regras de hora/data. **Não existe expediente** na configuração (o `business_*`/`day_*` + `schedule.py` foram removidos na 1.7.0, migration `006`): horário e dia da semana são CONDIÇÃO de retorno (`virtual.hora_atual`, `virtual.dia_semana`, `virtual.data_atual`) — um só lugar para decidir quem recebe, avaliado no disparo com dados frescos. `between` em campo de hora tem **wrap-around** (`16:00..07:30` num operador só).
- **UI**: UMA entrada no menu da engrenagem (`/retornos`, screen `config:false`, exige `plugin.retornos.view`) com abas internas **Configurações**, **Monitor** e **Eventos** ([static/retornos.js](assets/plugin_examples/retornos/static/retornos.js) + o construtor recursivo [static/RegraBuilder.js](assets/plugin_examples/retornos/static/RegraBuilder.js)). A aba **Eventos** (1.10.0) é o log do verificador que antes era o bloco "Últimos eventos" no fim do Monitor: rola dentro do próprio quadro (`max-h-[60vh]`, cabeçalho e rodapé fixos), filtra por tipo de evento e por conversa, e **pagina no servidor** — `GET /monitor/logs?evento&conversation_id&limit&offset` devolve `{items, total, limit, offset, eventos}` (o `total` vem de `repo.count_logs`, com a MESMA cláusula da página via `_log_where`; contar sobre a página diria "1 de 50" para sempre). Clicar no JSON de uma linha alterna truncado ↔ inteiro. A aba **Monitor** (1.11.0) tem, acima da tabela, filtros que recortam **no servidor** (`GET /monitor/controles`, nunca sobre a página carregada): status, **configuração**, **disparos** (campo digitável, casamento EXATO com `disparos_enviados`; `0` é filtro legítimo — só o campo VAZIO não filtra, então o teste é contra `None`, nunca contra a falsidade do valor) e o intervalo **`next_from`/`next_to`** sobre a coluna "Próximo" (`next_at`; agendamento sem próximo disparo fica de fora quando qualquer um dos dois é passado). Quem converte o DIA do calendário em epoch é o **navegador**, no fuso dele — o servidor só compara epoch, mesmo contrato do plano 99. O select de configuração sai de `GET /monitor/filtros` (`{configuracoes}`); a chamada falhando, ele degrada para "Todas" e a tabela continua. Os cartões do topo seguem sendo o total GLOBAL por status (`/monitor/stats`) — não acompanham o filtro nem a página. Desde a **1.12.0** a tabela também **pagina no servidor**, igual à aba Eventos: `GET /monitor/controles?…&limit&offset` devolve `{items, total, limit, offset}` (default 50, teto 1000), o `total` vem de `repo.count_controles` com a MESMA cláusula da página (`_controle_where`) e o `ORDER BY` termina em `c.id DESC` — os critérios de cima (`status='active'`, `next_at`, `updated_at`) empatam com facilidade e, sem desempate estável, a mesma linha apareceria em duas páginas ou em nenhuma. Antes disso a lista era truncada em 200 linhas **em silêncio**. Trocar filtro volta à página 1; página além do fim (cancelou a última linha) recua sozinha. O preview "passaria agora?" roda `rules.js` no cliente sobre o contexto real trazido por `POST /preview` — os tipos vêm de `tiposDeMetadata(metadata)`, não só do espelho estático, senão campo de plugin viraria "desconhecido" e o preview mentiria. No select de campo, os grupos contribuídos por plugin (`origem: "plugin"`) vêm depois dos do core, precedidos da divisória **"Configurações (outros plugins)"** (uma `<option disabled>`: `<optgroup>` não aninha). O **anexo já enviado é conferível**: o nome do arquivo (e o botão "Ver arquivo", que também cobre o caso da URL) abre o modal de [static/MediaPreview.js](assets/plugin_examples/retornos/static/MediaPreview.js) com a imagem, o player de áudio/vídeo ou — para PDF/formato sem player — o cartão com "Abrir / baixar". O modo de exibir vem da categoria REAL do arquivo (extensão), não do tipo da mensagem. **PDF não é embutido de propósito**: a CSP do painel manda `frame-ancestors 'none'` em TODA resposta, então `<iframe>`/`<object>` apontado para o próprio `/statics` é bloqueado (medido) — abrir em outra aba é navegação de topo e funciona. Anexo em **URL externa** também não renderiza (`img-src/media-src 'self'`) e cai no mesmo cartão, via `onError`. RBAC: `view`/`edit`/`monitor`.
- **Testes**: [tests/test_retornos_rules.py](tests/test_retornos_rules.py) (motor puro), [tests/test_retornos_plugin.py](tests/test_retornos_plugin.py) (app real: migrations, API, eventos, ciclo, monitor, RBAC), [tests/test_retornos_campos_de_plugins.py](tests/test_retornos_campos_de_plugins.py) (o seam de campos externos, com `retornos` + `protocolos` no mesmo app) e `node --test assets/plugin_examples/retornos/static/rules.test.js`.
...
| 2 | `assets/plugin_examples/<id>/` | Espelho transitório no core. GOWA ainda é bundled; alguns testes mistos/legados ainda dependem das outras fontes | não criar plugin novo aqui; remover gradualmente quando os contratos restantes forem desacoplados |
...
Testes do core que ainda precisam de uma fonte real usam [tests/plugin_test_utils.py](tests/plugin_test_utils.py): a resolução prefere `WHATSBOT_PLUGIN_SOURCE_ROOT/<id>/src`, cai no espelho transitório `assets/plugin_examples/<id>/` e só depois na instalação. Para contratos genéricos, prefira [tests/fake_provider.py](tests/fake_provider.py). Teste de costura deve usar [tests/support.py](tests/support.py) e o namespace canônico `whatsbot_plugins.<id>.*`.
