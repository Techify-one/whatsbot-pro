...
Plugins de exemplo bundled em `assets/plugin_examples/` (copiados na 1ª execução): `lembretes` (tools + routes + migrations + screen — anota lembretes pedidos pelo contato e mostra na tela com update via WebSocket) e `telegram` (canal completo via Bot API — channels + lifecycle long-poll + routes + screen `config:true`; recebe por long-poll ou webhook, com `/autoconfigure` que detecta domínio público HTTPS ao criar o canal). O `telegram` é bundled porque seu frontend vive no core ([web/static/js/components/ChannelsManager.js](web/static/js/components/ChannelsManager.js) + [api.js](web/static/js/services/api.js) chamam `/api/plugins/telegram/autoconfigure` e `/status`) — mantê-lo no repo evita que plugin e core saiam de sincronia.
