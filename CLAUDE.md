...
Falhando **qualquer um**, o recurso vai inteiro para o plugin — mesmo que o gancho disponível seja mais tosco que o ramo que você desenharia. Precedente conta como evidência: se outro plugin já resolve o mesmo problema pelo mesmo gancho em produção, o gancho está provado e o ônus da prova é de quem quer o ramo no core.

**Por que a regra existe (não é estética):** um plugin zero-core **elimina ordem de deploy**. Com mudança no core, subir o zip novo num core velho descarta eventos em silêncio e obriga a sequência "core primeiro, zip depois"; sem ela, importar o `.zip` basta. Critério de aceite de qualquer plugin: **instala e funciona num core da release anterior**.
...
| ~~`filter.media.unknown`~~ | 🚫 **MORTO (medido em 2026-07-31)** — o nome está em `KNOWN_FILTERS` ([plugins/events.py:139](plugins/events.py#L139)) mas **não existe `apply_filter("filter.media.unknown", …)` em lugar nenhum** do repo (o call site existiu em `755cb1a` e sumiu no refactor do webhook). Um plugin que o registre **nunca é chamado e nem leva WARNING**, porque o nome está no catálogo. **Hoje não há como um plugin reivindicar um media type novo** | — | — | — |
...
🚫 **Registrar um media type NOVO não é possível hoje.** O gancho que existia para isso (`filter.media.unknown`) está **morto**: declarado em `KNOWN_FILTERS`, sem call site nenhum — ver a linha dele na tabela de filters. O dispatch de inbound também é fechado (12 `kind` literais, sem `else` e sem log de "kind não reconhecido"), então um provider que devolva um `kind` desconhecido cai fora em silêncio.
