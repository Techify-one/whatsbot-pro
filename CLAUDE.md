...
- **IA por canal** (plano 21): overrides em `channels.config["ai"]` ([channels/ai_settings.py](channels/ai_settings.py), cache 30s). O gate é **global `auto_reply` → `ai_enabled` do canal → `ai_active` da conversa**.
- **Atendente padrão para novas conversas** (plano 71 · 152): UM campo, DUAS chaves exclusivas — `default_assignee_user_id` (humano ⇒ nasce atribuída + IA off) ou `default_assignee_agent_key` (agente de IA ⇒ nasce vinculada + IA **on**). ⚠️ A escolha do agente **vence** o `default_ai_enabled` (senão não faria nada), mas o master `ai_enabled` do canal a anula — e com ele off os agentes **nem aparecem** no seletor. Reabertura só reaplica em conversa **órfã**; com as duas chaves setadas o humano vence. [docs/CANAIS.md](docs/CANAIS.md). ⚠️ **Transcrição NÃO depende da IA do canal** — canal com a IA desligada continua descrevendo/transcrevendo, e `image_transcription_mode` **não é seedada** no config global (seedar faria uma instalação que desligou a descrição voltar a descrever).
...
- **Aviso "entrou/saiu do grupo"**: o participante viaja como o token `[[member:<tel>|<nome>]]` no `content` do `system_notice` e vira link para "Novo contato" ([systemMemberLinks.js](web/static/js/services/systemMemberLinks.js)). ⚠️ **LID NÃO é telefone** — sem telefone real conhecido o nome fica texto puro; e `_member_mark` (Python) e `MEMBER_RE` (JS) são o mesmo formato. [docs/UI_CONVERSA.md](docs/UI_CONVERSA.md)
- **Ver membros do grupo**: linha do `ContactInfoPanel` (só grupo) abre o popup [GroupMembersModal.js](web/static/js/components/contacts/GroupMembersModal.js); clicar num membro leva ao "Novo contato" pré-preenchido (`/contacts?createPhone=…`). ⚠️ O modal navega **por conta própria** (o overlay de Contatos faz `stopPropagation`, o interceptor do shell nunca veria o clique), o Esc dele exige `preventDefault` (contrato do Esc do hub) e membro só-LID **não** vira link. [docs/UI_CONVERSA.md](docs/UI_CONVERSA.md)
