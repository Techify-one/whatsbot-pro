...
- **`statics/` precisa de volume persistente no deploy**: `statics/` está no `.gitignore` E `.dockerignore`, e é criada vazia em runtime dentro do container. O `VOLUME ["/app/statics", ...]` do [Dockerfile](Dockerfile) declara um volume **anônimo**, que o Coolify **descarta ao recriar o container num redeploy** — então a mídia enviada pelo operador (`statics/senditems/`) e o cache de avatares (`statics/avatars/`) somem a cada deploy, dando 404 (`{"detail":"Not Found"}`) nas imagens já enviadas. Em dev (`linux_start.sh`, processo direto, sem container) isso não acontece. **Fix:** no Coolify, configurar **Persistent Storage** mapeando `/app/statics` (ou ao menos `/app/statics/senditems`) para um volume nomeado/host path. Avatares são cache auto-recuperável (re-baixados do GOWA), então o painel já cai num placeholder 200 em vez de 404 (rota `GET /statics/avatars/{name}` em [server/app.py](server/app.py)); imagem perdida no chat renderiza um placeholder "indisponível". Multi-réplica (não é o caso atual, single-container) exigiria storage compartilhado de verdade (volume por-nó não basta)
