...
- **Plugin `facebook_messenger`** ([assets/plugin_examples/facebook_messenger/](assets/plugin_examples/facebook_messenger/)): fonte importável quando empacotada pelo [builder de plugins](scripts/build_plugin_zips.py) (D10 — não auto-instalado; o zip local é gitignored e não vem no checkout). Credenciais `page_id`/`page_access_token`/`app_secret`/`verify_token`; configs `graph_api_version` (v25.0) e `human_agent_tag`; dedup por `page_id`; `contact_type` = `facebook`; screen `config:true` para copiar a URL de callback e assinar o webhook da Página (`POST /{page_id}/subscribed_apps`). Identidade do contato = **PSID page-scoped** (a chave `(channel_id, phone)` do core já cobre).
...
- **Bundling** ([plugins/bootstrap.py](plugins/bootstrap.py) `BUNDLED_AUTO_INSTALL = ("gowa",)`): fresh install copia **só GOWA**. As outras fontes ficam em `assets/plugin_examples/` para testes/build; seus `assets/channel_plugins/<id>-plugin.zip` são artefatos locais gitignored, gerados pelo builder, e só ficam importáveis na tela Plugins depois de serem gerados/distribuídos. Instalações existentes em `storages/plugins/` ficam intactas.
...
Plugin **auto-instalado** no fresh install (plano 33 D3, [plugins/bootstrap.py](plugins/bootstrap.py) `BUNDLED_AUTO_INSTALL`): **apenas `gowa`** (canal WhatsApp via GOWA, ATIVO por padrão). Os outros providers de canal — `telegram` (Bot API — channels + lifecycle long-poll + routes + screen `config:true`; recebe por long-poll ou webhook, com `/autoconfigure`) e `whatsapp_cloud` (WhatsApp Cloud API da Meta) — não são mais copiados no boot: são instalados por **Importar `.zip`**. O descriptor mantém a oferta/form/chips visuais alinhados com o plugin carregado; isso **não** sincroniza fonte, zip publicado e instalação. A **fonte** continua em `assets/plugin_examples/<id>/` (bancada de diff/teste e entrada do builder). Os `*-plugin.zip` de `assets/channel_plugins/` são artefatos locais **gitignored**: gere/verifique com [scripts/build_plugin_zips.py](scripts/build_plugin_zips.py), mas não confunda build local com publicação ou disponibilidade no clone/deploy. Os demais plugins de exemplo (`channel_test`, `lembretes`, `runtime_probe`) foram movidos para o **repositório de plugins do Pro** (`whatsbot-pro-plugins`), cada um em `plugins/<id>/` com `.zip` importável + `.json` de metadados, instalados sob demanda via `Importar (.zip)`.
...
|---|---|---|---|
| 1 | `assets/plugin_examples/<id>/` | Fonte versionada **no git deste repo** dos 8 plugins que o core embarca (`gowa`, `telegram`, `whatsapp_cloud`, `website`, `facebook_messenger`, `instagram`, `protocolos`, `melhorias`). Usada pelos testes, pra gerar os zips, e como **bancada** de todo movimento core→plugin (é onde o `git diff` e a suíte enxergam a mudança) | commit neste repo |
...
- Build reproduzível a partir da bancada: `venv/bin/python scripts/build_plugin_zips.py <id>`; `--all` descobre todos, `--check` compara sem escrever e `--list` só lista. O arquivo sai em `assets/channel_plugins/<id>-plugin.zip`, mas continua gitignored e precisa de uma etapa separada de publicação/distribuição.
...
Não rode duas suítes PostgreSQL em paralelo: cada processo chama [tests/pg.py](tests/pg.py) e recria o mesmo schema `public`. Para incluir também `tests/` embarcados em plugins instalados, rode `venv/bin/python -m pytest -q` **sem** passar `tests/`; o hook de coleta só acrescenta essas árvores na descoberta padrão.

Testes que precisam de uma fonte de plugin usam [tests/plugin_test_utils.py](tests/plugin_test_utils.py): a resolução procura primeiro `assets/plugin_examples/<id>/` e depois `storages/plugins/<id>/`, e o loader sintético preserva imports relativos. Teste de costura deve subir o app com [tests/support.py](tests/support.py) e, ao monkeypatchar código carregado, pegar o módulo canônico `whatsbot_plugins.<id>.*` com `loaded_plugin_module()` — carregar o mesmo arquivo por caminho cria outro objeto e pode dar falso-verde. Para contratos do core que só precisam de um provider, prefira [tests/fake_provider.py](tests/fake_provider.py) em vez de acoplar o teste a GOWA/Telegram/Cloud reais. Rotas autenticadas podem usar a fixture `authenticated_admin`, que remove sessão e usuário ao final.

Os testes inserem dados de teste (contatos, mensagens, tags, usage); o legado `tests/test_endpoints.py` validava **1.641 checagens** na medição de 2026-07-31, além da suíte pytest nativa, cobrindo:
...
- **Bootstrap de plugins**: as fontes de referência vivem em `assets/plugin_examples/<id>/` (trackeadas no git), mas `BUNDLED_AUTO_INSTALL` copia automaticamente **somente `gowa`** para `storages/plugins/gowa/`; não existe cópia genérica de todas as fontes quando a pasta está vazia. `telegram`/`whatsapp_cloud` e os demais são import-only: o zip em `assets/channel_plugins/` é artefato local gitignored, gerado pelo builder, e precisa ser distribuído/importado explicitamente. Se o usuário deletar um plugin import-only, ele não volta no próximo boot. Os plugins `channel_test`, `lembretes` e `runtime_probe` foram movidos para o **repositório de plugins do Pro** (`whatsbot-pro-plugins`; cada um em `plugins/<id>/` com `.zip` + `.json`). **Exceção (plano 52)**: o `gowa` bundled tem upgrade **version-aware** — se a versão em `assets/plugin_examples/gowa/plugin.yaml` for maior que a instalada, o boot substitui `storages/plugins/gowa/` pela cópia bundled (edições manuais nessa pasta são perdidas; logado como warning).
